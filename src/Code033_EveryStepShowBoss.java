import java.util.*;
     ...
     */
public class Code033_EveryStepShowBoss {
    //给购买次数最多的前K名用户颁奖
    ...
    public static class WhosYourDaddy {
        ...
        private Code032_HeapGreater<Customer> candHeap;
        private Code032_HeapGreater<Customer> daddyHeap;
        private final int daddyLimit;


        public WhosYourDaddy(int limit) {
            customers = new HashMap<Integer, Customer>();
            candHeap = new Code032_HeapGreater<>(new CandidateComparator());
            daddyHeap = new Code032_HeapGreater<>(new DaddyComparator());
            daddyLimit = limit;
        }

        // 当前处理i号事件，arr[i] -> id, op[i] -> buyOrRefund
        public void operate(int time, int id, boolean buyOrRefund) {
            ...
            } else if (candHeap.contains(c)) {//候选区存在该用户
                if (c.buy == 0) {
                    candHeap.remove(c);
                } else {
                    candHeap.resign(c);
